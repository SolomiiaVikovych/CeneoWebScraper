...
class Product:
    ...
    def calculate_stats(self):
        ...
        self.stats["recomendations"] = opinions.recommend.value_counts(dropna=False).reindex([True,False, np.nan],fill_value=0).to_dict()
...
class Opinion:
    ...
    def __init__(self,opinion_id="", author="",recommend=False,stars=0,content_pl="",pros_pl=[],cons_pl=[],up_votes=0,down_votes=0,published=None,purchased=None, content_en="", pros_en=[], cons_en=[]):
        ...
        self.publised = published
        self.purchased= purchased
        self.conten_en= content_en
    ...
    def translate(self):
        ...
        self.conss_en = [translate_data(cons) for cons in self.cons_pl]
        return self

    def transform(self):
            self.recommend = True if single_opinion['recommend']=="Polecam" else False if single_opinion['recommend']=="Nie polecam" else None
            self.stars = float(single_opinion['stars'].split("/")[0].replace(",","."))
            self.up_votes = int(single_opinion['up_votes'])
            self.down_votes = int(single_opinion['down_votes'])
            self.opinions.append(single_opinion)
            return self
